...
module Gush
  class Client
    ...
    def self.redis_connection(config)
      redis_parameters = config.redis_config || { url: config.redis_url }
      cached = (@@redis_connection.value ||= redis_parameters.merge({ connection: nil }))
      return cached[:connection] if !cached[:connection].nil?

      Redis.new(redis_parameters).tap do |instance|
        RedisClassy.redis = instance
        @@redis_connection.value = redis_parameters.merge({ connection: instance })
      end
    end
    ...
    def enqueue_job(workflow_id, job)
      ...
      worker_class.set(queue: queue).perform_async(*[workflow_id, job.name])
    end
